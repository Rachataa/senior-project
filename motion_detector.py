...
firebase = firebase.FirebaseApplication('https://peoplecount-7a371.firebaseio.com/', None)

sender_email = "madnessvv@gmail.com"
rec_email = firebase.get('/Email', 'setmail')
password = "deeppkvyosnfpalk"
subject = 'WARNING ROBBERY !!!'
message = "Encounter intruders in the area Other than the specified time. Please call Emergency 191"
...
def time():
    overtime = firebase.get('/Time', 'settime')
    return overtime
    ...
    while True:
        ...
        for c in foreground_objects:
            ...
            if closest_person_index is not None:
                lowest_closest_distance < LOWEST_CLOSEST_DISTANCE_THRESHOLD
                ...
                if change == ENTERED_STRING:
                    ...
                    x = date.today()
                    t = time.localtime()
                    y = time.strftime("%H:%M", t)
                    data = {'No': inside_count,
                            'Day': x,
                            'Time': y}
